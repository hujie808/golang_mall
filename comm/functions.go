...
func TimeUnix(i int64) time.Time {
	a := time.Unix(i, 0).Format(conf.SysTimeform)
	loc, _ := time.LoadLocation("Local") //重要：获取时区
	theTime, _ := time.ParseInLocation(conf.SysTimeform, a, loc)
	return theTime
}
...
//判断文件加是否存在
func IsExist(path string) bool { // banner/
	_, err := os.Stat(path)
	if err != nil {
		if os.IsExist(err) {
			return true
		...
		if os.IsNotExist(err) {
			err := os.MkdirAll(bootstrap.StaticAssets+path, os.ModePerm)
			if err != nil {
				log.Println("functions.go IsExist os.MkdirAll err=", err)
				return false
			}
			return false
		}
		log.Println("functions.go IsExist err=", err)
		return false
	}
	return true
}
...
//添加新的字段
func StructMapAdd(obj interface{}, keyName string, valueName interface{}) map[string]interface{} {
...
//Struct保留字段
func StructMapHold(obj interface{}, keyName ...string) []map[string]interface{} {
	...
	for _, o := range objs {
		...
		for i := 0; i < t.NumField(); i++ {
			for _, k := range keyName {
				//log.Println(data[t.Field(i).Name],k)
				if t.Field(i).Name == k {
					data[t.Field(i).Name] = v.Field(i).Interface()
				}
			}
